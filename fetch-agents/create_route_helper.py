...
def add_label_markers(image, labels, background_size):
    ...
    for label in labels:
        ...
        adjusted_x = (x - oldpoint1) * scale_factor_x + offset_x + 25
...
def shopping_order(label_positions, label_list):
...
def draw_route(image, route, label_positions, barriers):
    draw = ImageDraw.Draw(image)
    width, height = image.size
